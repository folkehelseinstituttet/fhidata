...
1. Clean the data
2. Replace NAs as appropriate for MSIS location numbers
3. Convert MSIS location numbers to location_code's using `fhidata::norway_locations_msis_to_fhidata`
4. Re-aggregate your data to different geographical levels to ensure that duplicates have now been removed
5. Pull out important dates
6. Create `multiskeleton`
7. Merge in the information you have at different geographical granularities
8. Aggregate up to higher geographical granularities
9. (If desirable) aggregate up to higher time granularities
...
# 2. Replace NAs as appropriate for MSIS location numbers
d_agg$day_municip[is.na(msis_location_municip), msis_location_municip := 9999]
d_agg$day_county[is.na(msis_location_county), msis_location_county := 99]

# 3. Convert MSIS location numbers to location_code's using `fhidata::norway_locations_msis_to_fhidata`
# you may also do redistricting (kommunesammenslaaing) at this point
d_agg$day_municip[, location_code := fhidata::norway_locations_msis_to_fhidata(msis_location_municip)]
d_agg$day_county[, location_code := fhidata::norway_locations_msis_to_fhidata(msis_location_county)]
